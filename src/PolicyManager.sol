...
contract PolicyManager is EIP712, ReentrancyGuard {
    ...
    ///
    /// @dev Exposed as a public getter for integrators.
    ...
    ///
    /// @dev **Permanent:** once uninstalled, a policyId can never be reinstalled. The specific combination of
    ...
    /// If the hook reverts and `msg.sender` is the bound account, the revert is swallowed and uninstallation succeeds;
    ...
    function isPolicyActiveNow(address policy, bytes32 policyId) external view returns (bool) {
        return _isPolicyActiveAt(policy, policyId, uint40(block.timestamp));
    }

    /// @notice True if the policy is installed, not uninstalled, and would be within its valid install window at the
    ///         given timestamp.
    ///
    /// @dev Useful for pre-flight checks where the caller needs to verify a policy will still be active at a future
    ///      timestamp (e.g., accounting for transaction confirmation delay).
    ///
    /// @param policy Policy contract address.
    /// @param policyId Policy identifier.
    /// @param timestamp The timestamp (seconds) to check against the validity window.
    function isPolicyActiveAt(address policy, bytes32 policyId, uint40 timestamp) external view returns (bool) {
        return _isPolicyActiveAt(policy, policyId, timestamp);
    }
    ...
    ///
    /// @return newPolicyId Deterministic policy identifier for the new binding.
    function _replace(ReplacePayload calldata payload) internal returns (bytes32 newPolicyId) {
        ...
        if (newRecord.uninstalled) revert PolicyIsDisabled(newPolicyId);
    ...
    ///
    /// @param policyRecord Storage pointer to the old policy record (already loaded by `_replace`).
    ...
    ///      The caller (`_replace`) must have already validated `!policyRecord.installed` and `!policyRecord.uninstalled`.
    ///
    /// @param policyRecord Storage pointer to the new policy record (already loaded by `_replace`).
    /// @param policyId Pre-computed policy identifier for the new binding (avoids redundant `getPolicyId`).
    /// @param binding New binding to install (carries its own `policyConfig`).
    /// @param newPolicyReplaceData Policy-defined replacement payload forwarded to the new policy's `onReplace` hook.
    ...
    function _installForReplace(
        ...
        bytes32 policyId,
        PolicyBinding calldata binding,
        bytes calldata newPolicyReplaceData,
        address otherPolicy,
        bytes32 otherPolicyId
    ) internal {
    ...
    /// @param timestamp The timestamp (seconds) to check against the validity window.
    ///
    /// @return True if the policy is installed, not uninstalled, and would be within its valid install window at the
    ///              given timestamp.
    function _isPolicyActiveAt(address policy, bytes32 policyId, uint40 timestamp) internal view returns (bool) {
        PolicyRecord storage policyRecord = policies[policy][policyId];
