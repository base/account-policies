...
interface Policy {
    ...
    function onInstall(bytes32 policyId, address account, bytes calldata policyConfig, address caller) external;

    /// @notice Policy hook invoked during revocation.
    /// @dev Called by `PolicyManager` after the binding has been marked revoked.
    function onRevoke(bytes32 policyId, address account, address caller) external;
