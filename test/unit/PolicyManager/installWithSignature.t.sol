...
contract InstallWithSignatureTest is PolicyManagerTestBase {
    ...
    /// @notice Reverts when a non-zero deadline has expired.
    ///
    /// @param deadlineSeed Seed used to derive a deadline in the past.
    /// @param configSeed Seed used to build the committed config bytes.
    /// @param salt Salt used to derive the policyId.
    function test_reverts_whenDeadlineExpired(uint256 deadlineSeed, bytes32 configSeed, uint256 salt) public {
        vm.warp(WARP_BASE_TIMESTAMP);
        uint256 deadline = bound(deadlineSeed, 1, uint256(WARP_BASE_TIMESTAMP) - 1);

        bytes memory policyConfig = abi.encode(_safeConfigSeed(configSeed));
        PolicyManager.PolicyBinding memory binding = _binding(address(installPolicy), policyConfig, salt);
        bytes memory userSig = _signInstall(binding, deadline);

        vm.expectRevert(abi.encodeWithSelector(PolicyManager.DeadlineExpired.selector, block.timestamp, deadline));
        policyManager.installWithSignature(binding, userSig, deadline, bytes(""));
    }

    /// @notice Succeeds when deadline is zero (no expiry).
    function test_succeeds_whenDeadlineIsZero() public {
        bytes memory policyConfig = abi.encode(bytes32(0));
        PolicyManager.PolicyBinding memory binding = _binding(address(installPolicy), policyConfig, 0);
        bytes memory userSig = _signInstall(binding, 0);

        bytes32 policyId = policyManager.installWithSignature(binding, userSig, 0, bytes(""));
        (bool installed,,,,) = policyManager.policies(address(installPolicy), policyId);
        assertTrue(installed);
    }

    /// @notice Succeeds when deadline is in the future.
    ///
    /// @param deadlineOffset Seed used to derive a deadline in the future.
    function test_succeeds_whenDeadlineInFuture(uint256 deadlineOffset) public {
        vm.warp(WARP_BASE_TIMESTAMP);
        uint256 deadline = bound(deadlineOffset, uint256(WARP_BASE_TIMESTAMP) + 1, type(uint256).max);

        bytes memory policyConfig = abi.encode(bytes32(0));
        PolicyManager.PolicyBinding memory binding = _binding(address(installPolicy), policyConfig, 0);
        bytes memory userSig = _signInstall(binding, deadline);

        bytes32 policyId = policyManager.installWithSignature(binding, userSig, deadline, bytes(""));
        (bool installed,,,,) = policyManager.policies(address(installPolicy), policyId);
        assertTrue(installed);
    }

    /// @notice Reverts with invalid signature when deadline in signature doesn't match deadline parameter.
    function test_reverts_whenDeadlineMismatch() public {
        vm.warp(WARP_BASE_TIMESTAMP);
        uint256 signedDeadline = WARP_BASE_TIMESTAMP + 1000;
        uint256 suppliedDeadline = WARP_BASE_TIMESTAMP + 2000;

        bytes memory policyConfig = abi.encode(bytes32(0));
        PolicyManager.PolicyBinding memory binding = _binding(address(installPolicy), policyConfig, 0);
        bytes memory userSig = _signInstall(binding, signedDeadline);

        vm.expectRevert(PolicyManager.InvalidSignature.selector);
        policyManager.installWithSignature(binding, userSig, suppliedDeadline, bytes(""));
    }
    ...
    function test_reverts_whenExecutionDataProvided_andPolicyOnExecuteReverts() public {
        ...
        policyManager.installWithSignature(binding, userSig, 0, executionData);
    }
    ...
    function test_allowsMultipleInstalls_withDifferentSalts(uint256 saltA, uint256 saltB) public {
        ...
        bytes32 policyIdA = policyManager.installWithSignature(bindingA, userSigA, 0, bytes(""));
        bytes32 policyIdB = policyManager.installWithSignature(bindingB, userSigB, 0, bytes(""));
